...
| Branch/PR named explicitly (user, handoff, or session setup) | Use it directly - naming is permission, overrides any pre-assigned/current branch, no confirmation.<br>- User's own reference wins on conflict (e.g. "PR63" vs a session-assigned scratch branch) - resolve to that artifact's actual branch, never assume the pre-assigned one; not "obvious" per step 4 |
