...
- Other delegated tasks: `model: sonnet` when a wrong answer is cheap to catch; `model: opus` for large-scale refactoring or migration, structurally novel design, or a costly/irreversible error. Current prices and limits -> `claude-api`, never memory.
- Never recommend a mid-session model or effort switch: each model, and on most models each effort level, has its own prompt cache, so the switch re-reads the whole history uncached. Another model needed mid-task -> subagent (parent cache unaffected).
