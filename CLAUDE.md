...
| Branch/PR named explicitly (user, handoff, or session setup) | Use it directly - naming is permission, overrides any pre-assigned/current branch, no confirmation.<br>- User's own reference (e.g. "PR63") counts as this naming and wins over session-setup pre-assignment when they conflict - resolve to that artifact's actual branch, don't default to the pre-assigned one |
