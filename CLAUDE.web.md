...
## Every turn
1. Identify the task. Topic is .NET/C#/Blazor, or user asks -> use dotnet-skills. Topic is web/frontend design, or user asks -> use extract-design-system and web-design-guidelines.
2. Scan local skills, >=1% relevant -> invoke + announce ("Using [skill] to [purpose]"). Always check regardless of scan: any file op or multi-file task -> `token-efficiency`; unfamiliar code area or need the bigger picture -> `zoom-out`; about to state an unverified factual/technical/procedural claim -> `verifying-sources`. Every skill invoked this turn, from any step (2, 4, or 5) -> same one-line announce, no silent invocations.
