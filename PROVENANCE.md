...
| 2026-07-27 | Bootstrap | Leave dotnet-skills uninstalled for now | `2c3eb5a` | Yes - installs gated on topic match (.NET/C#/Blazor), avoids unneeded installs - retired (`d6647d7`, 2026-09-08): the .NET topic gate itself moved to `README.md` "Optional skills", nothing left in Bootstrap to except it from |
| 2026-08-29 | Bootstrap | `npx skills add homeassistant-ai/skills@home-assistant-best-practices` (unconditional, every session) | `0427a12` | Yes - "covers safe HA config editing... adopt this one and evaluate the gap in practice" - retired (`d6647d7`, 2026-09-08): unconditional install of a stack-specific skill cost every session regardless of relevance; moved to `README.md` "Optional skills" for a Home Assistant project to opt into |
...
| 2026-08-23 | Every turn | Stamp the reply with local time | `5f0124b` | Yes - replaces a fabricated token estimate the old rule forced when the harness exposes none - retired (`6c296d3`, 2026-09-22): redundant with the session's own timestamp display |
| 2026-08-23 | Every turn | Report a token figure only when the harness exposes one | `5f0124b` | Yes - same commit as the row above - retired (`a1d5001`, 2026-09-08): the exposed figure is a static remaining-budget number, not a live per-session usage count, so the rule either always skips or reports a misleading figure |
...
| 2026-08-23 | Every turn | Gap since the previous stamp over the cache lifetime -> say this turn reprocessed the whole history | `5f0124b` | Yes - same commit as the row above (missed by the ledger's original seeding pass) - retired (`d6647d7`, 2026-09-08): same flavor as the token-figure rule above, assumes a harness-tracked cache-lifetime/previous-stamp state the model cannot verify from within the conversation |
| 2026-07-27 | Every turn | Topic is .NET/C#/Blazor, or user asks -> npx skills add dotnet-skills and use token-dotnet | `2c3eb5a` | Yes - "Move aaronontheweb/dotnet-skills under Conditional, gated on .NET/C#/Blazor topic" (missed by the ledger's original seeding pass) - retired (`d6647d7`, 2026-09-08): moved to `README.md` "Optional skills" - most consumer projects are never .NET, so the gate cost every session regardless |
| 2026-07-27 | Every turn | Topic is web/frontend design, or user asks -> npx skills add extract-design-system and web-design-guidelines | `a079d46` | Yes - "installing every session regardless of whether the task ever touched design work... moved to trigger-gated installs" (missed by the ledger's original seeding pass) - retired (`d6647d7`, 2026-09-08): same reasoning one level further - moved to `README.md` "Optional skills" instead of a topic gate in the always-loaded file |
| 2026-08-02 | Every turn | Test design with >=3 combinable parameters -> npx skills add pict-test-designer | `4239161` | Yes - "topic gates for pict-test-designer (MIT, remote install)... on explicit request only" (missed by the ledger's original seeding pass) - retired (`d6647d7`, 2026-09-08): moved to `README.md` "Optional skills" |
| 2026-08-02 | Every turn | User explicitly asks for parallel/sub-agents -> npx skills add dispatching-parallel-agents and subagent-driven-development | `4239161` | Yes - same commit/rationale as the row above (missed by the ledger's original seeding pass) - retired (`d6647d7`, 2026-09-08): moved to `README.md` "Optional skills" |
...
| 2026-07-25 | Error handling | Validate-gate question unanswered -> wait silently (hook/notification noise isn't a reply) | `097ac94` | No (origin not logged) - part of a broader Error handling table addition, no individual reason given - changed (`9896a6b`, 2026-09-12): dropped only "wait silently" (the harness cannot emit true silence, so the agent produced a meta-comment narrating the silence instead - noisier than the repeat it was meant to suppress); kept the "hook/notification noise isn't a reply" disambiguation, reattached to "don't act" |
| 2026-09-02 | Communication | No delta -> no reply, not even a placeholder, including a repeating turn-end reminder | `ef59a02` | Yes - RETROSPECTIVE.md 2026-09-01: a turn-end reminder with no delta still drew a near-identical reply two turns running - retired (`9896a6b`, 2026-09-12): same failure as the Error handling row above, same fix |
| 2026-09-22 | Every turn | Any file op or multi-file task -> `token-file-ops` | `bff8e44` | Yes - `token-efficiency` deleted: its file-op rules duplicated `token-file-ops` (unique ones merged there), its model/effort/monitor sections were stale, user-controlled, or covered by `claude-api`/`handoff` |
| 2026-09-22 | Agents | Other delegated tasks: sonnet when a wrong answer is cheap to catch, opus for large-scale refactoring/migration, novel design, or costly/irreversible error; prices and limits from `claude-api` | `bff8e44` | Yes - subagent `model` is the only model choice the agent makes; moved from the deleted `token-efficiency`, rewritten against platform.claude.com choosing-a-model (2026-09-22) instead of hardcoded benchmark gaps |
| 2026-09-22 | Agents | Never recommend a mid-session model or effort switch; another model mid-task -> subagent | `bff8e44` | Yes - code.claude.com prompt-caching: each model (and on most models each effort level) has its own cache; replaces `token-efficiency`'s false "switching is free" |
