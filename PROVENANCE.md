...
| 2026-07-27 | Bootstrap | Leave dotnet-skills uninstalled for now | `2c3eb5a` | Yes - installs gated on topic match (.NET/C#/Blazor), avoids unneeded installs - retired (this commit, 2026-09-08): the .NET topic gate itself moved to `README.md` "Optional skills", nothing left in Bootstrap to except it from |
| 2026-08-29 | Bootstrap | `npx skills add homeassistant-ai/skills@home-assistant-best-practices` (unconditional, every session) | `0427a12` | Yes - "covers safe HA config editing... adopt this one and evaluate the gap in practice" - retired (this commit, 2026-09-08): unconditional install of a stack-specific skill cost every session regardless of relevance; moved to `README.md` "Optional skills" for a Home Assistant project to opt into |
...
| 2026-08-23 | Every turn | Gap since the previous stamp over the cache lifetime -> say this turn reprocessed the whole history | `5f0124b` | Yes - same commit as the row above (missed by the ledger's original seeding pass) - retired (this commit, 2026-09-08): same flavor as the token-figure rule above, assumes a harness-tracked cache-lifetime/previous-stamp state the model cannot verify from within the conversation |
| 2026-07-27 | Every turn | Topic is .NET/C#/Blazor, or user asks -> npx skills add dotnet-skills and use token-dotnet | `2c3eb5a` | Yes - "Move aaronontheweb/dotnet-skills under Conditional, gated on .NET/C#/Blazor topic" (missed by the ledger's original seeding pass) - retired (this commit, 2026-09-08): moved to `README.md` "Optional skills" - most consumer projects are never .NET, so the gate cost every session regardless |
| 2026-07-27 | Every turn | Topic is web/frontend design, or user asks -> npx skills add extract-design-system and web-design-guidelines | `a079d46` | Yes - "installing every session regardless of whether the task ever touched design work... moved to trigger-gated installs" (missed by the ledger's original seeding pass) - retired (this commit, 2026-09-08): same reasoning one level further - moved to `README.md` "Optional skills" instead of a topic gate in the always-loaded file |
| 2026-08-02 | Every turn | Test design with >=3 combinable parameters -> npx skills add pict-test-designer | `4239161` | Yes - "topic gates for pict-test-designer (MIT, remote install)... on explicit request only" (missed by the ledger's original seeding pass) - retired (this commit, 2026-09-08): moved to `README.md` "Optional skills" |
| 2026-08-02 | Every turn | User explicitly asks for parallel/sub-agents -> npx skills add dispatching-parallel-agents and subagent-driven-development | `4239161` | Yes - same commit/rationale as the row above (missed by the ledger's original seeding pass) - retired (this commit, 2026-09-08): moved to `README.md` "Optional skills" |
